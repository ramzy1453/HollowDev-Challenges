<script>
	...
	async function handleSubmit() {
		const { data, success, message } = await login({ email, password });
		if (success) {
			auth.user = data.user;
...
<div class="flex h-screen border border-black">
	...
	>
		<div class="flex max-w-md flex-1 flex-col items-center justify-center">
			<h1 class="mb-2 text-2xl font-semibold">Login to your account</h1>
			<p class="text-sm">Enter your email below to access your account</p>
			...
			<form class="mt-6 w-full space-y-2" onsubmit={handleSubmit}>
				...
				<Button class="w-full" variant="default" type="submit">Sign In</Button>
			</form>
			...
			<Button variant="outline" disabled={isLoading} class="w-full font-medium">
				{#if isLoading}
